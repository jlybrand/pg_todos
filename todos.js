...
// Error here if another user has a todolist named X another user cannont name a todolist X. This is because the todolists table has a unique value for the todolist_title column. 
...
requiresAuthentication,
  catchError(async (req, res, next) => {
    let todoListId = req.params.todoListId;
    let deletedList = await res.locals.store.deleteTodoList(+todoListId);

    if (!deletedList) throw new Error("Not found.");
...
app.post("/lists/:todoListId/edit",
  ...
  catchError(async (req, res) => {
    ...
    const rerenderEditList = async () => {
      let todoList = await store.loadTodoList(+todoListId);
      if (!todoList) throw Error("Not found.");
    ...
    try {
      ...
      if (store.isUniqueConstaintViolation(error)) {
        req.flash("error", "The list title must be unique");
        rerenderEditList();
      } else {
        throw error;
